...
function removeReview(bookId, reviewId) {
  return get(bookId)
      .then(((currBook) => {
          const idx = currBook.reviews.findIndex(review => review.id === reviewId)
...
function _createBooks() {
  ...
  if (!books || !books.length) {
    books = gBooks
    // books = []
    // books.push(_createBook('metus hendrerit', {amount: 85, currencyCode: 'USD', isOnSale: false}))
    // books.push(_createBook('morbi', {amount: 85, currencyCode: 'USD', isOnSale: false}))
    // books.push(_createBook('at viverra venenatis', {amount: 85, currencyCode: 'USD', isOnSale: false}))
    utilService.saveToStorage(BOOK_KEY, books)
  }
}

function _createBook(title, listPrice = { amount: 85, currencyCode: 'USD', isOnSale: false }) {
  const book = getEmptyBook(title, listPrice)
  book.id = utilService.makeId()
  book.img = utilService.getRandomIntInclusive(1, 20)
  return book
}